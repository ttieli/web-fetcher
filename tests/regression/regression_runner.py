...
class RegressionRunner:
    ...
    def run_test(self, test: URLTest) -> TestResult:
        ...
        try:
            ...
            # Fetch the URL
            # 抓取 URL
            # fetch_html_with_retry now returns (html, metrics, url_metadata)
            _result = fetch_html_with_retry(
                url=test.url,
                ua=ua,
            ...
            if len(_result) == 3:
                html, fetch_metrics, _url_meta = _result
            else:
                html, fetch_metrics = _result
