...
### 5. CDP交互式采集能力 (点击/滚动) / CDP Interactive Capabilities (Click/Scroll)

**问题 / Issue:**
许多现代网页（如新闻专题、社交媒体）需要点击"加载更多"或滚动到底部才能显示完整内容。当前的 `CDPFetcher` 仅支持页面加载后的静态快照。
Many modern websites (e.g., news topics, social media) require clicking "Load More" or scrolling to the bottom to reveal full content. The current `CDPFetcher` only supports a static snapshot after page load.

**解决方案 / Solution:**
利用 CDP 的 `Runtime.evaluate` 接口注入 JavaScript 来模拟用户交互。
Leverage CDP's `Runtime.evaluate` interface to inject JavaScript for simulating user interactions.

**实施计划 / Implementation Plan:**
1.  **扩展 `CDPFetcher` / Extend `CDPFetcher`:**
    *   添加 `click(selector: str)` 方法：执行 `document.querySelector(selector).click()`。
    *   添加 `scroll_to_bottom()` 方法：执行 `window.scrollTo(0, document.body.scrollHeight)`。
2.  **模板支持 / Template Support:**
    *   在 YAML 模板中增加 `interactions` 配置段落，允许定义抓取前的动作序列。
    *   示例：
        ```yaml
        interactions:
          - type: click
